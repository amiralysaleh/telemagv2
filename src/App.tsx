...
import React, { useEffect, useState } from 'react';
import { MessageCard } from './components/MessageCard';
import { Search, Filter, RefreshCw, Copy, Check, Bell, X, Globe, Heart } from 'lucide-react';
import { Message } from './types';
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans" dir="rtl">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="relative p-2 text-zinc-500 hover:text-blue-600 bg-zinc-100 dark:bg-zinc-800/50 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors mr-1 sm:mr-2"
                title="اطلاعیه‌ها"
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white border-2 border-white dark:border-zinc-900">
                    {unreadCount}
                  </span>
                )}
              </button>
      ...
      {showNotification && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowNotification(false)}>
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl max-w-md w-full overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
              ...
              <button onClick={() => setShowNotification(false)} className="p-1.5 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="p-5 space-y-4 text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">
              ...
              <p>
                برای دسترسی پایدارتر، وب‌سایت روی دو دامنه با دیتاسنترهای مجزا قرار گرفته است. پیشنهاد می‌کنیم از این صفحه <strong>اسکرین‌شات بگیرید</strong> تا در صورت بروز مشکل یا آپدیت نشدن یکی از دامنه‌ها، از آدرس جایگزین استفاده کنید:
              </p>
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-800/30 flex flex-col gap-2" dir="ltr">
                <a href="https://mag.runflare.run" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 font-medium hover:underline flex items-center gap-2">
                  <Globe size={16} /> mag.runflare.run
                </a>
                <a href="https://mag2.runflare.run" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 font-medium hover:underline flex items-center gap-2">
                  <Globe size={16} /> mag2.runflare.run
                </a>
              </div>
              <p>
                همچنین برای حمایت از بقا و توسعه وب‌سایت، یا درخواست اضافه شدن کانال‌های جدید، می‌توانید از طریق لینک زیر اقدام کنید.❤️
              </p>
              <div className="bg-rose-50 dark:bg-rose-900/20 p-3 rounded-xl border border-rose-100 dark:border-rose-800/30" dir="ltr">
                <a href="https://daramet.com/amiraly" target="_blank" rel="noopener noreferrer" className="text-rose-600 dark:text-rose-400 font-medium hover:underline flex items-center gap-2">
                  <Heart size={16} /> daramet.com/amiraly
                </a>
              </div>
            </div>
            <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 flex justify-end">
              <button onClick={() => setShowNotification(false)} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl transition-colors shadow-sm">
                متوجه شدم
              </button>
            </div>
          </div>
